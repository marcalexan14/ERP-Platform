...
- **JWT sessions stay "valid" forever if you reuse the same `AUTH_SECRET` across a database you've reset.** Auth.js's JWT strategy never checks the database to validate the token itself — only when resolving `session.user.id` to real data (e.g. `getCurrentOrganization`). So swapping to a fresh/empty database while keeping the same `AUTH_SECRET` leaves old browser cookies decoding successfully to a user id that no longer exists, surfacing as a confusing "No organization found for this user" instead of an auth failure. Sign out (or clear cookies) after resetting your dev database, or roll `AUTH_SECRET` when you do.
